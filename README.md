...
2. **⚡ IT-Auszubildende (Fachinformatiker AE/SI/DP/DVS, IT-Systemelektroniker, Kaufleute IT-Systemmanagement)**:
   ...
   - **RAG Semantic Cache & Vector Similarity Studio (`RagSemanticCacheLab.jsx` & `src/utils/ragSemanticCacheEngine.js`)**: Optimierungs-Studio für moderne Enterprise-KI- und RAG-Pipelines. Vektorielles Ähnlichkeits-Caching mit Cosine-Similarity-Schwellenwert. Schnelle Index-Lookups (<10ms) umgehen teure und langsame LLM-Inferenz, sparen 100% Token-Kosten und minimieren Latenzen bei semantisch ähnlichen Nutzeranfragen mit 65 XP Belohnung.
...
* **🛡️ SQL Transaction Isolation & ACID Studio (`SqlIsolationLab.jsx` & `src/utils/sqlIsolationEngine.js`)**:
  ...
  * **Interaktive Gegenüberstellung der 4 ANSI-SQL Isolationslevel**:
    * **Read Uncommitted**: Maximale Concurrency, jedoch Risiko von Geisterdaten.
    * **Read Committed (Standard in PostgreSQL)**: Statement-Level MVCC-Snapshots, verhindert Dirty Reads.
    * **Repeatable Read**: Transaction-Level Snapshot Isolation, verhindert Non-Repeatable Reads und Phantom Reads.
    * **Serializable (SSI / Serializable Snapshot Isolation)**: Strikteste Stufe mit SIREAD Locks und Dependency-Graph-Tracking, verhindert Write Skew.
  * **Simulation aller 4 Transaktionsanomalien mit Step-by-Step Timeline**:
    * **Dirty Read**: Lesen uncommitteter Daten nach späterem ROLLBACK.
    * **Non-Repeatable Read (Fuzzy Read)**: Veränderte Werte bei wiederholtem SELECT auf dieselbe Zeile.
    * **Phantom Read**: Neu aufgetauchte Zeilen bei wiederholten Bereichsabfragen (Range Queries).
    * **Write Skew**: Parallele Schreiboperationen verletzen globale Integritätsregeln trotz Snapshot-Isolation.
  * IHK-Prüfungsmatrix und 50 XP Belohnung bei Abschluss aller Szenarien.

* **⚡ DGUV Vorschrift 3 & VDE Elektro-Prüfstudio (`DguvV3ElektronikLab.jsx` & `src/utils/dguvV3ElektronikEngine.js`)**:
  * Praxisorientiertes IT-Infrastruktur- und Elektrotechnik-Studio für **IT-System-Elektroniker (ITSE AP2 Bereich 2)** und **Fachinformatiker Systemintegration (FISI Lernfeld 2)**.
  * **Schutzklassen nach DIN EN 61140 (VDE 0140-1)**:
    * Schutzklasse I (Schutzerdung $\color{amber}⏚$, Servernetzteile, Kaltgerätestecker).
    * Schutzklasse II (Schutzisolierung $\color{amber}回$, Notebook-Netzteile, Eurostecker).
    * Schutzklasse III (Schutzkleinspannung $\color{amber}⬖$, PoE bis 57V DC, SELV/PELV).
  * **Geräteprüfung nach DIN VDE 0701-0702**:
    * Schutzleiterwiderstand $R_{PE}$: Grenzwert max. $0,3\,\Omega$ (bis 5 m) $+ 0,1\,\Omega$ je weitere 7,5 m (max. $1,0\,\Omega$).
    * Isolationswiderstand $R_{ISO}$: Mindestanforderung $\ge 1,0\,\text{M}\Omega$ (SK I) bzw. $\ge 2,0\,\text{M}\Omega$ (SK II).
  * **RCD / FI Personenschutzprüfung nach DIN VDE 0100-410**:
    * Bemessungsfehlerstrom $I_{\Delta n} = 30\,\text{mA}$ mit Auslösestrom-Toleranzbereich ($15\,\text{mA}$ bis $30\,\text{mA}$).
    * Maximale Abschaltzeit $t_a \le 400\,\text{ms}$ im TN-Netz bzw. $t_a \le 200\,\text{ms}$ im TT-Netz.
  * **RZ-USV Batteriedimensionierung & Notstrom-Autonomie**:
    * Formel: $\text{Laufzeit} = \frac{\text{Kapazität (Ah)} \times \text{Spannung (V)} \times \eta}{\text{Serverlast (W)}} \times 60\,\text{Minuten}$ mit 60 XP Belohnung.

* **🎓 Volle IHK AP2-Berufsabdeckung im Prüfungssimulator (`ExamSimulator.jsx` & `src/data/examData.js`)**:
  * Vollständige Abdeckung aller 5 IT-Ausbildungsberufe nach offizieller Ausbildungsordnung (AO 2020):
    * **FIAE**: Anwendungsentwicklung (Softwarearchitektur, OOP, Clean Code, SQL).
    * **FISI**: Systemintegration (Routing, Subnetting, Firewalls, DNS/DHCP, Serverdienste).
    * **FIDP (Neu)**: Daten- und Prozessanalyse (ETL/ELT-Pipelines, Data Lineage, Machine Learning, Governance).
    * **FIDV (Neu)**: Digitale Vernetzung (Cyber-Physische Systeme, IIoT, MQTT, OPC UA, OT/IT-Sicherheit).
    * **ITSE (Neu)**: IT-System-Elektroniker (DGUV Vorschrift 3, RCD-Abschaltung, Leitungsdimensionierung).
  * Erweiterter Fragenkatalog (Fragen 26–31) mit detaillierten IHK-Musterlösungen und Begründungen.

* **⚡ SQL Window Functions & Analytics Studio (`SqlWindowFunctionsLab.jsx` & `src/utils/sqlWindowFunctionsEngine.js`)**:
  * Didaktisches Modern Data Engineering- und SQL-Analytics-Studio nach ANSI SQL:2003 Standard (PostgreSQL, SQLite 3.25+, MySQL 8.0+).
  * **Analytische Fensterfunktionen im interaktiven Vergleich**:
    * `ROW_NUMBER()`: Fortlaufende Zeilennummerierung innerhalb von Partitionen.
    * `RANK()`: Rangfolge mit Lücken bei Gleichständen (z. B. 1, 1, 3).
    * `DENSE_RANK()`: Lückenlose Rangfolge (z. B. 1, 1, 2).
    * `NTILE(k)`: Einteilung in Quartile/Terzile für Daten-Bucketizing.
    * `LEAD()` & `LAG()`: Zugriff auf nachfolgende und vorhergehende Datensätze ohne teure Self-JOINs.
    * Running Totals: Kumulierende Summen (`SUM(...) OVER (PARTITION BY ... ORDER BY ... ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)`).
  * **Dynamische ANSI SQL Query Generator** mit 1-Klick-Kopierfunktion und 65 XP Belohnung.

* **🐙 ArgoCD GitOps & Cluster Sync Studio (`ArgoCdGitOpsLab.jsx` & `src/utils/argoCdGitOpsEngine.js`)**:
  * Cloud-Native Continuous Delivery und Kubernetes GitOps-Simulation.
  * **Git als Single Source of Truth**:
    * Desired State (Git Repository) vs. Live State (Kubernetes Cluster).
    * Drift-Erkennung (`OutOfSync` vs. `Synced`) bei manuellen Eingriffen via `kubectl`.
    * Automatisches Self-Healing, Rollout von neuen Releases (v2.5.0) und Auto-Pruning verwaister Cluster-Ressourcen mit 65 XP Belohnung.

* **📐 Vektor-Mathematik & Embedding-Distanz Studio (`VectorMathEmbeddingLab.jsx` & `src/utils/vectorMathEngine.js`)**:
  * Mathematisches Fundament moderner Vektordatenbanken (Pinecone, Qdrant, Milvus) und KI-Retrieval-Pipelines.
  * **Vier Distanzmetriken im Live-Vergleich**:
    * **Cosine Similarity** ($\frac{A \cdot B}{\|A\| \|B\|}$) & geometrischer Schnittwinkel in Grad ($0^\circ$ bis $180^\circ$).
    * **Euklidische Distanz (L2-Abstand)**: Direkter geometrischer Abstand im Raum.
    * **Manhattan Distanz (L1-Abstand)**: Achsenparallele City-Block Distanz.
    * **Skalarprodukt (Dot Product)**: Schnelle Projektion bei L2-normalisierten Einheitsvektoren.
  * Interaktive 4-Dimensionen-Slider und Normalisierungs-Presets mit 65 XP Belohnung.

* **📋 IHK Projektantrag & Zeitplan-Kollisions-Linter (`IhkProjectProposalLab.jsx` & `src/utils/ihkProjectProposalEngine.js`)**:
  * Offizielles Prüfungsmodul nach AO 2020 für FIAE (80h), FISI (40h), FIDP (80h) und IT-SE (40h).
  * **Gantt-Zeitplan-Kollisionsprüfung**: Erkennt unlogische Phasenreihenfolgen (Dokumentation vor Entwicklung), Lücken von mehr als 5 Tagen und Zeitbereichs-Inkonsistenzen.
  * **K.O.-Kriterien-Prüfung**: Gewährleistung von Entscheidungsspielräumen, Wirtschaftlichkeitsanalysen und DSGVO Art. 32 TOM.
...
* **🔑 JWT Security & Algorithm Confusion Attack Studio (`JwtConfusionLab.jsx` & `src/utils/jwtConfusionEngine.js`)**:
  ...
  * **Asymmetrisch (RS256) vs. Symmetrisch (HS256) Key-Confusion**:
    * Schwachstellen-Analyse: Wenn ein Backend-Server die Signaturüberprüfung flexibel anhand des im Header übergebenen `alg`-Attributs durchführt und denselben Verifikationsschlüssel übergibt, kann ein Angreifer das Token fälschen.
    * Der Angreifer nimmt den **öffentlichen RSA-Public-Key** des Servers (der weltweit bekannt oder über JWKS öffentlich abrufbar ist), ändert den Header auf `{"alg": "HS256"}` und signiert die manipulierten Claims (z. B. `{"role": "admin"}`) mit dem öffentlichen Schlüssel als HMAC-Secret!
    * Ein ungeschützter Server interpretiert den Public Key fälschlicherweise als HMAC-Passwort und validiert das manipulierte Admin-Token erfolgreich.
  * **None-Algorithm Bypass & Exploit**:
    * Demonstration veralteter oder fehlkonfigurierter Bibliotheken, die `{"alg": "none"}` akzeptieren und Tokens ohne Signatur validieren.
  * **Live-Simulation & Verteidigungsmechanismen**:
    * Umschaltbare Server-Absicherung (Enforce RS256 Whitelist, Strip Keys, Strict Alg Validation).
    * Interaktiver Exploit-Generator (User-zu-Admin Privilegieneskalation), Verifikations-Console und Belohnung mit 65 XP.

* **🛡️ Linux SELinux & AppArmor Mandatory Access Control (MAC) Studio (`LinuxMacSelinuxLab.jsx` & `src/utils/linuxMacSelinuxEngine.js`)**:
  * Didaktisches Kernel-Sicherheits- und Access-Control-Studio für Fachinformatiker Systemintegration (FISI) und Security-Architekten.
  * **DAC vs. MAC Konfrontation**:
    * Discretionary Access Control (DAC: `chmod`/`chown`): Der Dateibesitzer oder Root entscheidet frei. Wird ein Webserver (`nginx`/`httpd`) kompromittiert, hat der Angreifer bei Root-Rechten unbeschränkten Zugriff auf das gesamte Dateisystem.
    * Mandatory Access Control (MAC: SELinux Type Enforcement & AppArmor Profile): Das Betriebssystem erzwingt strikte Richtlinien basierend auf Sicherheitskontexten (`user:role:type:level`). Selbst ein als `root` laufender Prozess im Kontext `httpd_t` darf nicht auf `shadow_t` oder fremde Verzeichnisse zugreifen.
  * **SELinux vs. AppArmor Architekturen**:
    * SELinux: Inode-basiertes Labeling mit Type Enforcement Matrix (`allow httpd_t var_log_t:file { read append };`).
    * AppArmor: Pfadbasiertes Profiling (`/etc/apparmor.d/usr.sbin.apache2`) mit Sub-Profilen und Whitelisting.
  * **Enforcing vs. Permissive Modus & Live AVC Audit Logs**:
    * Interaktiver Umschalter zwischen Enforcing (striktes Blockieren) und Permissive (Audit-Logging ohne Blockierung zur Regelerstellung via `audit2allow`).
    * Realistischer AVC Denial Audit Log-Viewer (`type=AVC msg=audit(...): avc: denied { read } for pid=... scontext=system_u:system_r:httpd_t:s0 tcontext=system_u:object_r:shadow_t:s0 tclass=file permissive=0`) mit 65 XP Belohnung.

* **🔒 DNS Privacy Inspector: DoH & DoT vs. Port 53 (`DnsPrivacyLab.jsx` & `src/utils/dnsPrivacyEngine.js`)**:
  * Tiefgehende Netzwerksicherheits- und Protokoll-Inspektion nach modernsten IETF-Standards.
  * **Drei Übertragungsprotokolle im Vergleich**:
    * Standard DNS (Port 53 UDP/TCP): Klartextübertragung. Jeder Router, ISP und Man-in-the-Middle kann angefragte Domains mitlesen, manipulieren oder DNS-Sperren erzwingen.
    * DNS-over-TLS (DoT, RFC 7858, Port 853): Dedizierter TLS-Tunnel auf TCP-Ebene. Verschlüsselt die Domain-Abfragen, der Port 853 signalisiert jedoch weiterhin klar DNS-Verkehr.
    * DNS-over-HTTPS (DoH, RFC 8484, Port 443): Kapselt DNS-Wire-Format (`application/dns-message`) in regulären HTTPS/HTTP/2/HTTP/3-Traffic. Für Firewalls und ISPs ununterscheidbar von regulärem Web-Surfen.
  * **Wire-Format Hex-Dump & ISP-Eavesdropping-Erkennung**:
    * Interaktiver Domain-Query-Inspector mit Anzeige des DNS-Header-Wire-Formats (ID, Flags, QDCOUNT, QNAME-Labels).
    * ISP-Sniffer-Simulation, die visualisiert, wie Metadaten, Server-IPs und SNI im Klartext exponiert oder durch TLS/DoH geschützt werden mit 60 XP Belohnung.

* **💰 IHK WISO Liquiditätsgrade & Working Capital Studio (`WisoLiquiditaetLab.jsx` & `src/utils/wisoLiquiditaetEngine.js`)**:
  * Praxisorientiertes Bilanzanalyse- und Zahlungsfähigkeits-Studio für Wirtschafts- und Sozialkunde (WISO) in der IHK Abschlussprüfung (AP2).
  * **Die drei klassischen Liquiditätsgrade**:
    * **Liquidität 1. Grades (Cash Ratio / Barliquidität)**: $\frac{\text{Flüssige Mittel}}{\text{Kurzfristiges Fremdkapital}} \times 100\%$ (IHK-Richtwert: $\ge 20\%$).
    * **Liquidität 2. Grades (Quick Ratio / Einzugsbedingte Liquidität)**: $\frac{\text{Flüssige Mittel} + \text{Kurzfristige Forderungen}}{\text{Kurzfristiges Fremdkapital}} \times 100\%$ (IHK-Richtwert: $\ge 100\%$).
    * **Liquidität 3. Grades (Current Ratio / Umsatzbedingte Liquidität)**: $\frac{\text{Umlaufvermögen}}{\text{Kurzfristiges Fremdkapital}} \times 100\%$ (IHK-Richtwert: $\ge 150\%$).
  * **Net Working Capital (NWC) & Insolvenzrechtliche Bewertung**:
    * $NWC = \text{Umlaufvermögen} - \text{Kurzfristige Verbindlichkeiten}$.
    * IHK-konforme Warnung und Handlungsempfehlungen bei drohender Zahlungsunfähigkeit nach InsO § 17 mit 60 XP Belohnung.

* **🧠 RAG Semantic Cache & Vector Similarity Studio (`RagSemanticCacheLab.jsx` & `src/utils/ragSemanticCacheEngine.js`)**:
  * Didaktisches KI-Architektur- und Kostenoptimierungs-Studio für Fachinformatiker Anwendungsentwicklung (FIAE) und AI Engineers.
  * **Vektorielles Semantic Caching**:
    * Statt bei jeder leicht umformulierten Nutzerfrage teure und langsame LLM-Inferenz zu starten, erzeugt ein Embedding-Modell einen hochdimensionalen Vektor der Eingabe.
    * Ein Vector Store vergleicht die Anfrage mittels Cosine Similarity gegen gespeicherte Q&A-Paare.
  * **Cosine Similarity Schwellenwert & Zero-Cost Hits**:
    * Einstellbarer Cosine-Threshold (z. B. 0.85): Liegt der Score darüber, erfolgt ein sofortiger **Cache Hit** in <10ms Latenz bei 0$ LLM-Tokenkosten.
    * Liegt der Score darunter, schlägt der Cache fehl (**Cache Miss**) und die Anfrage wird an das LLM weitergeleitet.
    * Interaktiver Editor zum Einlernen neuer Referenz-Prompts mit 65 XP Belohnung.
...
- **Neu**: `src/components/Content/SqlIsolationLab.jsx` & `src/utils/sqlIsolationEngine.js` — SQL Transaction Isolation & ACID Studio: Interaktive Erforschung der 4 ANSI-SQL Isolationsstufen (Read Uncommitted, Read Committed, Repeatable Read, Serializable SSI) und dynamische Simulation von Dirty Reads, Non-Repeatable Reads, Phantom Reads und Write Skew inklusive technischer MVCC- und SIREAD-Erklärungen mit 50 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 5 Unit-Tests abgesichert.
- **Neu**: `src/components/Content/DguvV3ElektronikLab.jsx` & `src/utils/dguvV3ElektronikEngine.js` — DGUV Vorschrift 3 & VDE Elektro-Prüfstudio: Prüfung elektrischer Schutzmaßnahmen für IT-System-Elektroniker (ITSE AP2) und Fachinformatiker (FISI LF 2). Schutzklassen I–III, Schutzleiterwiderstand $R_{PE}$ nach Leitungslänge ($0,3\,\Omega$ + $0,1\,\Omega$/7,5m), Isolationswiderstand $R_{ISO}$ ($1,0\,\text{M}\Omega$ / $2,0\,\text{M}\Omega$), RCD 30 mA Personenschutzprüfung ($t_a \le 400\,\text{ms}$ TN / $200\,\text{ms}$ TT) und USV-Batterieautonomie-Rechner mit 60 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 5 Unit-Tests abgesichert.
- **Erweitert**: `src/components/Content/ExamSimulator.jsx` & `src/data/examData.js` — Volle IHK AP2-Berufsabdeckung nach Ausbildungsordnung 2020:
  - Ergänzung der dedizierten Prüfungsmodi für **FIDP** (Daten- und Prozessanalyse), **FIDV** (Digitale Vernetzung) und **ITSE** (IT-System-Elektroniker).
  - Neuer Fragenkatalog (Fragen 26–31) mit detaillierten IHK-Musterlösungen zu ELT vs. ETL, Data Lineage, MQTT QoS, OPC UA, DGUV Vorschrift 3 Grenzwerten und RCD-Personenschutz (30 mA).
- **Typisierung & Engine-Härtung**:
  - `src/utils/ihkProjectProposalEngine.js`: Absicherung gegen ungültige Berufs-Keys und Typisierung (`// @ts-check`).
  - `src/utils/jwtConfusionEngine.js`: JSDoc Parameter-Typen (`base64UrlEncode`, `generateSimulatedSignature`, `forge*`).
  - `src/utils/wisoCapitalValueEngine.js`: Typisierung von `npvFn` und `dynamicPaybackPeriod` (`tsc --noEmit` fehlerfrei).
- **Routing & Integration**: Vollständige Registrierung beider neuer Studios in `src/App.jsx` (Lazy Loading & `activeLabElement` Switch-Tabelle mit Routen `sql_isolation_lab` und `dguv_v3_lab`), `src/components/Content/LabsDashboard.jsx`, `src/components/Navigation/Navbar.jsx` und `src/components/Navigation/CommandPaletteModal.jsx`.
- **Smoke Tests & Komponenten-Integrität**: `src/components/componentsIntegrity.test.jsx` um Smoke-Tests für beide neuen Studios erweitert (63/63 Komponenten-Tests bestanden) und `allLabsSmoke.test.jsx` erfolgreich über alle 197 Studios durchlaufen.
- **Test-Suite & Qualität**: **1021 bestandene Unit- & Integrationstests** in **140 Test-Dateien** (100% Erfolgsquote, +10 Tests!), **0 Oxlint-Fehler / 0 Warnungen** über 539 Quelldateien (`oxlint src --deny-warnings`), `tsc --noEmit` fehlerfrei, PWA Produktions-Build erfolgreich und alle `size-limit`-Vorgaben eingehalten.

### Version 3.50.0 (SQL Window Functions, ArgoCD GitOps, Vektor-Mathematik & IHK Antrags-Linter Edition)

- **Neu**: `src/components/Content/SqlWindowFunctionsLab.jsx` & `src/utils/sqlWindowFunctionsEngine.js` — SQL Window Functions & Analytics Studio: ANSI SQL:2003 Standard-Funktionen (`ROW_NUMBER()`, `RANK()`, `DENSE_RANK()`, `NTILE()`, `LEAD()`, `LAG()`, Running Totals mit `SUM(...) OVER (...)`) und dynamischer SQL-Code-Generator mit 65 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 3 Unit-Tests abgesichert.
- **Neu**: `src/components/Content/ArgoCdGitOpsLab.jsx` & `src/utils/argoCdGitOpsEngine.js` — ArgoCD GitOps & Cluster Sync Studio: Cloud-Native Declarative Delivery. Git als Single Source of Truth, Erkennung von Out-of-Sync Drifts (`kubectl scale`), automatische Self-Healing Synchronisation, Auto-Prune verwaister Ressourcen und Rollouts neuer Releases mit 65 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 3 Unit-Tests abgesichert.
- **Neu**: `src/components/Content/VectorMathEmbeddingLab.jsx` & `src/utils/vectorMathEngine.js` — Vektor-Mathematik & Embedding-Distanz Studio: Mathematisches Fundament von Vektordatenbanken (Pinecone, Qdrant, Milvus). Cosine Similarity, geometrischer Schnittwinkel in Grad, Euklidische L2-Distanz, Manhattan L1-Abstand, Skalarprodukt und L2-Vektornormalisierung mit 65 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 3 Unit-Tests abgesichert.
- **Erweitert**: `src/utils/ihkProjectProposalEngine.js` & `ihkProjectProposalEngine.test.js` — Zeitplan-Kollisions-Linter für den IHK Projektantrag (AO 2020) mit automatischer Erkennung unlogischer Phasenreihenfolgen, unplausiblen Projektpausen und Ergänzung des Berufsbildes IT-System-Elektroniker (IT-SE 40h).
- **Routing & Integration**: Vollständige Registrierung aller 3 neuen Studios in `src/App.jsx` (Lazy Loading & `activeLabElement` Switch-Tabelle), `src/components/Content/LabsDashboard.jsx`, `src/components/Navigation/Navbar.jsx` und `src/components/Navigation/CommandPaletteModal.jsx`.
- **Smoke Tests & Komponenten-Integrität**: `src/components/componentsIntegrity.test.jsx` um Smoke-Tests für alle 3 neuen Studios erweitert (61/61 Komponenten-Tests bestanden) und `src/App.routing.test.jsx` erfolgreich mit 307/307 Routen-Tests durchlaufen.
- **Test-Suite & Qualität**: **1011 bestandene Unit- & Integrationstests** in **138 Test-Dateien** (100% Erfolgsquote, +9 Tests!), **0 Oxlint-Fehler / 0 Warnungen** über 533 Quelldateien (`oxlint src --deny-warnings`), `tsc --noEmit` fehlerfrei, optimierter PWA Produktions-Build und alle `size-limit`-Vorgaben eingehalten.
...
- **Neu**: `src/components/Content/LinuxMacSelinuxLab.jsx` & `src/utils/linuxMacSelinuxEngine.js` — Linux SELinux & AppArmor Mandatory Access Control (MAC) Studio: Didaktische Gegenüberstellung von DAC (`chmod`/`chown`, Root-Privilegien) und MAC (Type Enforcement, Sicherheitskontexte `user:role:type:level`). Interaktiver Modus-Umschalter (Enforcing vs. Permissive), Simulation von Privilege Escalation Versuchen kompromittierter Daemons (`httpd_t` Zugriff auf `shadow_t`), Live-Generierung authentischer AVC Denial Audit Logs (`/var/log/audit/audit.log`) und Lösungsfindung via `audit2allow` mit 65 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 3 Unit-Tests abgesichert.
- **Neu**: `src/components/Content/DnsPrivacyLab.jsx` & `src/utils/dnsPrivacyEngine.js` — DNS Privacy Inspector (DoH & DoT vs. Port 53): Tiefgehende Netzwerksicherheits-Analyse nach RFC 8484 (DNS-over-HTTPS auf Port 443) und RFC 7858 (DNS-over-TLS auf Port 853) im Vergleich zu unverschlüsseltem UDP/TCP Port 53. Interaktive Abfrage-Simulation, DNS Wire-Format Hex-Dump Inspektion (ID, Flags, QDCOUNT, QNAME Labels) und ISP Eavesdropping & Censorship Audit mit 60 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 3 Unit-Tests abgesichert.
- **Neu**: `src/components/Content/WisoLiquiditaetLab.jsx` & `src/utils/wisoLiquiditaetEngine.js` — IHK WISO Liquiditätsgrade & Working Capital Studio: Bilanzanalyse und Zahlungsfähigkeit nach offiziellem IHK-Standard für AP2 WISO. Berechnung von Liquidität 1. Grades (Cash Ratio, Barliquidität $\ge 20\%$), Liquidität 2. Grades (Quick Ratio, einzugsbedingte Liquidität $\ge 100\%$), Liquidität 3. Grades (Current Ratio, umsatzbedingte Liquidität $\ge 150\%$) und Net Working Capital (NWC = Umlaufvermögen - kurzfristiges FK). Automatische insolvenzrechtliche Warnung bei drohender Zahlungsunfähigkeit nach InsO § 17 mit 60 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 2 Unit-Tests abgesichert.
- **Neu**: `src/components/Content/RagSemanticCacheLab.jsx` & `src/utils/ragSemanticCacheEngine.js` — RAG Semantic Cache & Vector Similarity Studio: KI-Infrastruktur- und Kostenoptimierungs-Studio für Enterprise-RAG-Systeme. Vektorielles Ähnlichkeits-Caching mit 8-dimensionalem semantischen Embedding-Raum und Cosine-Similarity-Schwellenwert. Schnelle Index-Lookups (<10ms) umgehen teure und langsame LLM-Inferenz, sparen 100% Token-Kosten und minimieren Latenzen bei semantisch ähnlichen Nutzeranfragen. Interaktiver Editor zum Einlernen neuer Referenz-Prompts mit 65 XP Belohnung. Vollständig typgeprüft (`// @ts-check`) und mit 4 Unit-Tests abgesichert.
- **Design & Styling Upgrade**: Modernes Design-System in `global.css` mit harmonisierten HSL- und Slate-Paletten, samtigen Glasmorphismus-Verläufen (`--gradient-glass`), tiefen Schatten-Ebenen (`--shadow-card-hover`), abgerundeten Ecken (`--radius-sm` bis `--radius-xl`) und optimierten Primär-Akzenten (`#4338ca` Indigo 700 / `#818cf8` Indigo 400).
- **Responsive Layout & Mobile First**: Ergänzung des Breakpoints für Smartphones (<480px, <768px, <1024px) mit automatischer 1-Spalten-Skalierung im `.grid-responsive`, optimierten 44px-Touch-Targets nach WCAG 2.5.5, flüssiger Typografie (`clamp()`), flexiblen Innenabständen (`.container-responsive`) und Überlappungsschutz für die mobile Fixed-Bottom-Navigation.
- **Barrierefreiheit (WCAG 2.1 AAA, LRS & Farbenblindheit)**:
  - **Lese-Rechtschreib-Schwäche (Dyslexie)**: Gezielte Unterstützung durch die Spezialschrift *Atkinson Hyperlegible* mit erweitertem Zeichenabstand (`0.07em`), vergrößertem Wortabstand (`0.16em`) und 1.95 Zeilenhöhe zur Vermeidung von Buchstabenverwechslungen.
