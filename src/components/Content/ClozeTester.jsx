...
import { FileText, CheckCircle2, AlertCircle, RefreshCw, Award } from 'lucide-react';
...
export default function ClozeTester({ userState, onCompleteCloze }) {
  ...
  // Render Cloze text with inline dropdowns
        ...
        return (
          <span key={idx} style={{ display: 'inline-block', margin: '0 4px' }}>
            <select
              ...
              style={{
                ...
                background: isSubmitted 
                  ? (isCorrect ? 'rgba(16, 185, 129, 0.25)' : 'rgba(244, 63, 94, 0.25)')
                  : 'var(--bg-tertiary)',
                color: isSubmitted
                  ? (isCorrect ? 'var(--accent-green)' : 'var(--accent-rose)')
                  : 'var(--text-main)',
                border: isSubmitted
                  ? (isCorrect ? '1px solid var(--accent-green)' : '1px solid var(--accent-rose)')
            ...
            {isSubmitted && !isCorrect && (
              <span style={{ fontSize: '0.78rem', color: 'var(--accent-green)', marginLeft: '4px' }}>
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '60px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <h2 style={{ fontSize: '1.8rem', fontWeight: '800', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <FileText size={28} color="var(--accent-purple)" /> Interaktive Lückentexte & IHK-Wissen
        </h2>
          ...
          {CLOZE_TESTS.map(test => (
            <button
              ...
              style={{
                ...
                border: activeTestId === test.id ? '1px solid var(--accent-purple)' : '1px solid var(--border-color)',
                cursor: 'pointer',
                whiteSpace: 'nowrap'
              }}
            >
      ...
      <div className="glass-panel" style={{ padding: '32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <span className="badge badge-purple">{currentTest.category}</span>
          <span style={{ fontSize: '0.85rem', color: 'var(--accent-amber)', fontWeight: '700' }}>
            Belohnung: +{currentTest.xpReward} XP
          </span>
        </div>

        <h3 style={{ fontSize: '1.4rem', fontWeight: '700', marginBottom: '12px' }}>{currentTest.title}</h3>
        ...
        <div style={{ marginTop: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          {!isSubmitted ? (
            ...
            >
              <Award size={18} /> Lückentext Auswerten
            </button>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div style={{
                ...
                alignItems: 'center',
                gap: '8px',
                color: score >= 75 ? 'var(--accent-green)' : 'var(--accent-rose)',
                fontWeight: '700',
                fontSize: '1.1rem'
              }}>
                {score >= 75 ? <CheckCircle2 size={24} /> : <AlertCircle size={24} />}
